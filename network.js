/**
 * geneRelate — PPI Network Visualization
 * Static Layout (Pre-calculated) with Smooth Zoom/Pan
 * Features:
 *  - Pre-calculated Force-Directed Layout (No animation)
 *  - Auto-Fit to Viewport
 *  - Smooth Zoom & Pan
 *  - Interactive Drag (Geometry only, no physics)
 *  - Unique Gradient IDs
 */
...
function buildPPINetworkSVG(resolvedGenes, ppiData, infoData, scoreThreshold, getNameFn, taxid) {
    ...
    for (const n of nodes) {
        n.x = cx + (Math.random() - 0.5) * 50; // Start localized
        n.y = cy + (Math.random() - 0.5) * 50;
        n.vx = 0; n.vy = 0;
        n.r = n.isQuery || n.isHub ? 14 : 7;
    }

    // --- Pre-calculate Layout (Synchronous) ---
    ...
    for (let iter = 0; iter < 300; iter++) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            for (let j = i + 1; j < nodes.length; j++) {
                ...
                // Collision
                const rSum = nodes[i].r + nodes[j].r + 4; // Extra spacing
                if (dist < rSum) {
        ...
        nodes.forEach(n => {
            ...
            const maxV = 15 * alpha; // Lower limit as it cools
...
function renderNetworkViewer(nodes, edges, width, height, taxid) {
    ...
    svg.style.userSelect = 'none'; // Prevent text selection on drag
    ...
    if (scale > 2) scale = 2; // Don't zoom in too much for small graphs

    // Center it
    const midX = (minX + maxX) / 2;
    const midY = (minY + maxY) / 2;
    // Translate = Center of SVG - (Center of Content * Scale)
    ...
    // Setup Interaction (Zoom/Pan/Drag)
...
function setupInteraction(svg, container, initX, initY, initScale, nodes, nodeElements, edgeElements) {
    ...
    svg.addEventListener('wheel', (e) => {
        ...
        // Apply zoom factor
        // Using exponent for smooth scaling
        // delta usually +/- 100
        const delta = -e.deltaY;
        const factor = Math.pow(1.001, delta); // Gentle exponential zoom
        ...
        // Adjust translate to keep point under mouse stationary
        // NewTranslate = Screen - Point * NewScale
    ...
    function onMove(e) {
        ...
        if (isPanning) {
            ...
            // This requires scanning edges. For <200 edges this is fine.
